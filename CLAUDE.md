...
- **`tokenEditorSave` 存檔時若沒有選音檔案，會呼叫 `app.js` 的 `autoSynthesizeAudio(parsed)`
  ...
  **重要：`findAudioCandidates` 回傳的是「所有」同音候選，不是只有第一個，因為辭典資料裡
  ...
  不是還沒修的 bug。
  **`autoSynthesizeAudio` 找候選（整詞、逐音節皆然）一定要「先試變調後讀音，找不到任何
  候選再退而求其次試本調讀音」兩層都試過，不能只試變調後一種。** 已經踩過的坑（使用者
  實測回報「看護」khàn-hōo 合成失敗）：詞內變調後的調值常常根本不是任何字的「本調」——
  「看」khàn 本調第3聲，這個詞裡變調後變成第2聲，但教育部辭典裡沒有任何字本調剛好是
  `khan2`（`khan` 骨架＋第2聲根本不是一個會被單獨錄音的本調組合），這種情況
  `findAudioCandidates` 對變調後讀音直接回傳空陣列（不是「有候選但位元組抓不到」那種
  已經處理過的情況，是候選數量本身就是 0），若只試變調後讀音就整個詞放棄，會讓很多合法
  複合詞（組成的字各自都有本調錄音，只是教育部沒收錄這個詞本身）也無法自動合成——實際上
  「看」khàn（本調第3聲）、「護」hōo（本調第7聲）都各自有真人錄音。現在整詞跟逐音節兩處
  都是先呼叫 `findAudioCandidates(sandhiSylls)`／`findAudioCandidates([sandhiSylls[i]])`
  試變調後讀音，`findWorkingAudioUrl` 回傳 falsy（不管是候選是空陣列、還是候選都抓不到
  位元組）時，才用 `parsed`／`parsed[i]`（未變調的本調讀音）再試一次；兩層都失敗才算
  真的沒有可用候選。以後改這段時，這個「兩層 fallback」的順序（先變調後、再本調）不要
  拿掉，也不要反過來（先試本調會讓聽感更差的候選優先蓋過本來找得到的、更自然的變調後
  候選）。**存檔前一定要先呼叫
