...
- **v1.16.1 — 2026-09-12**
  - 修正補上讀音面板「破音字切換」選項按鈕看不出來對應哪個字的問題：`app.js` 的 `renderTokenEditorChoices`（`tokenEditorChoices`）原本按鈕文字只有 `讀音：trs` 這種格式，對同一個漢字底下的破音字（`toCommonTokens` 的 hanzi 分支，`token.entries`，例如食的白讀 tsia̍h／文讀 si̍t）沒問題，因為漢字欄位本身就顯示那個字；但對羅馬字反查出的同音字候選（rom 分支，`token.hanziMatches`，每筆本來就帶 `hanzi` 欄位）完全沒用到這個資訊，導致好幾個不同字但剛好同音（例如 tíng：戥／等／頂／鼎）的候選按鈕文字長得一模一樣，使用者完全看不出來要選的其實是不同的字
  - 現在按鈕文字改成有 `alt.hanzi` 時一律加在最前面（例如「戥　讀音：tíng」「等　文：tíng」），沒有 `alt.hanzi`（同字破音字）的情況維持原樣不變
- **v1.16.0 — 2026-09-12**
  - 修正補上讀音時自動合成音檔常常找不到的問題：`app.js` 的 `autoSynthesizeAudio()` 原本只用「這個詞自己詞內變調後」的讀音去找候選（整詞、逐音節皆然），找不到候選就直接放棄；但變調後的調值常常根本不是任何字的本調（例如「看護」khàn-hōo，「看」khàn 本調第3聲，詞內變調後變成第2聲，而教育部辭典裡沒有任何字本調剛好是第2聲的 khan，這不是「有 id 但抓不到位元組」，是根本沒有候選字可試）
  - 改成整詞／逐音節各自先試變調後讀音，找不到任何候選（或候選都抓不到位元組）才退而求其次改用本調（原本輸入的調，未變調）讀音再試一次；兩層都試過還是沒有才真的放棄整個詞。「看護」現在可以正確借用「看」khàn＋「護」hōo 兩個本調錄音合成
- **v1.15.0 — 2026-09-12**
