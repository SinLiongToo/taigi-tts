...
- **變調後（連讀參考）**：解析結果下方多一列「變調後」，套用台語標準連讀變調規則（規則來源見下方「資料來源」），把本調換算成連續講話時的實際調值——純參考顯示，不影響播放或其他欄位。查無讀音、只顯示〔數字調〕占位的詞一樣能套用，因為變調只需要本調數字，不需要真的查到字。分組簡化為「遇標點就斷開」，沒有處理輕聲、疊字變調等特殊規則。
...
- 播放的是各自獨立錄製的單字音檔依序接續，沒有連續語流的連讀變調，聽感上不會等同一句流暢口語；「變調後」列可以看到連讀時理論上的調值，但**播放的語音本身不會套用變調**（音檔是固定的本調錄音，沒辦法即時變調播放）。
